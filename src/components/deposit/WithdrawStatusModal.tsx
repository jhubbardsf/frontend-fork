...
const WithdrawStatusModal: React.FC<WithdrawStatusModalProps> = ({
    ...
    const selectedInputAsset = useStore((state) => state.selectedInputAsset) as ValidAsset;
    ...
    useEffect(() => {
        if (!isConnected) {
            setIsWaitingForCorrectNetwork(isAwaitingConnection);
        } else {
            if (isWaitingForCorrectNetwork && chainId === selectedInputAsset.chainId) {
                setIsWaitingForCorrectNetwork(false);
            }
        }
    }, [isWaitingForCorrectNetwork, chainId, selectedInputAsset.chainId]);
            ...
            <ModalContent
                ...
                sx={{
                    '@keyframes breathe': {
                        '0%, 100%': {
                            filter: isError
                                ? 'drop-shadow(0px 0px 30px rgba(183, 6, 6, 0.3))'
                                : 'drop-shadow(0px 0px 30px rgba(6, 64, 183, 0.4))',
                        },
                        '50%': {
                            filter: isError
                                ? 'drop-shadow(0px 0px 40px rgba(183, 6, 6, 0.5))'
                                : 'drop-shadow(0px 0px 50px rgba(6, 64, 183, 0.6))',
                        },
                    },
                }}>
                <ModalHeader
                            ...
                            <Flex
                                ...
                                <Flex justify='space-between ' w='100%' align='center'>
                                    <Text
                                        ...
                                        Amount{' '}
                                        {formatUnits(
                                            BigNumber.from(selectedSwapToManage.depositAmount).toString(),
                                            BITCOIN_DECIMALS,
                                        )}
                                    </Text>
                                </Flex>
